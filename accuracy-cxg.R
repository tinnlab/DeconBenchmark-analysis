...
dataFiles <- list.files('./data/sim-cxg')
allConfig <- expand.grid(dataFiles, methods)

# 8 cores, 16GB per job, 12GB softlimit, 12 concurrent jobs, max runtime of 12 hours

dir.create('./results/accuracy-cxg', showWarnings = FALSE, recursive = TRUE)
lapply(seq_len(nrow(allConfig)), function(i) {
    dataFile <- paste0('./data/sim-cxg/', as.character(allConfig[i,]$Var1))
    method <- as.character(allConfig[i,]$Var2)

    resFile <- paste0('./results/accuracy-cxg/', method, '_', as.character(allConfig[i,]$Var1))
...
rawRes <- lapply(datasets, function(dataset) {
    groundTruthFile <- paste0('./data/sim-cxg/', dataset, '.rds')
    groundTruth     <- readRDS(groundTruthFile)$bulkRatio %>% t()

    res <- lapply(methods, function(method) {
        # message(dataset, method)
        resFile <- paste0('./results/accuracy-cxg/', method, '_', dataset, '.rds')
...
saveRDS(allRes, file = './results/accuracy-cxg/allRes.rds')