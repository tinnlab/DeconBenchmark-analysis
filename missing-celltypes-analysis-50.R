...
dataFiles <- list.files('./data/sim-cxg-missing50')
allConfig <- expand.grid(dataFiles, methods[1])

# 8 cores, 16GB per job, 12GB softlimit, 12 concurrent jobs, max runtime of 12 hours

dir.create('./results/accuracy-cxg-missing50', showWarnings = FALSE, recursive = TRUE)
lapply(seq_len(nrow(allConfig)), function(i) {
    dataFile <- paste0('./data/sim-cxg-missing50/', as.character(allConfig[i,]$Var1))
    method <- as.character(allConfig[i,]$Var2)

    resFile <- paste0('./results/accuracy-cxg-missing50/', method, '_', as.character(allConfig[i,]$Var1))
...
rawRes <- lapply(datasets, function(dataset) {
    groundTruthFile <- paste0('./data/sim-cxg-missing50/', dataset, '.rds')
    groundTruth     <- readRDS(groundTruthFile)$bulkRatio %>% t()

    res <- lapply(methods, function(method) {
        # message(dataset, method)
        resFile <- paste0('./results/accuracy-cxg-missing50/', method, '_', dataset, '.rds')
...
saveRDS(allRes, file = './results/accuracy-cxg-missing50/allRes.rds')