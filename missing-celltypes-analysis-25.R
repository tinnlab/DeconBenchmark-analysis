...
dataFiles <- list.files('./data/sim-cxg-missing25')
allConfig <- expand.grid(dataFiles, methods[1])

# 8 cores, 16GB per job, 12GB softlimit, 12 concurrent jobs, max runtime of 12 hours

dir.create('./results/accuracy-cxg-missing25', showWarnings = FALSE, recursive = TRUE)
lapply(seq_len(nrow(allConfig)), function(i) {
    dataFile <- paste0('./data/sim-cxg-missing25/', as.character(allConfig[i,]$Var1))
    method <- as.character(allConfig[i,]$Var2)

    resFile <- paste0('./results/accuracy-cxg-missing25/', method, '_', as.character(allConfig[i,]$Var1))
...
rawRes <- lapply(datasets, function(dataset) {
    groundTruthFile <- paste0('./data/sim-cxg-missing25/', dataset, '.rds')
    groundTruth     <- readRDS(groundTruthFile)$bulkRatio %>% t()

    res <- lapply(methods, function(method) {
        # message(dataset, method)
        resFile <- paste0('./results/accuracy-cxg-missing25/', method, '_', dataset, '.rds')
        ...
        colnames(P) <- mapRes[colnames(P),]$to
        cnU <- intersect(colnames(P), colnames(groundTruth))


        res$P <- P[, colnames(groundTruth[,cnU])]
        res$groundTruth <- t(groundTruth[,cnU])
...
allRes <- lapply(datasets, function(dataset) {
    ...
    lapply(methods, function(method) {
        ...
        corrS <- mean(evaluateResult(res$P, res$groundTruth, method = "sample"), na.rm = TRUE)
        corrC <- mean(evaluateResult(res$P, res$groundTruth, method = "CT"), na.rm = TRUE)
        mae   <- mean(evaluateResult(res$P, res$groundTruth, metric = "MAE"), na.rm = TRUE)
        spw   <- evaluateResult(res$P, res$groundTruth, metric = "samplePairwise")
...
saveRDS(allRes, file = './results/accuracy-cxg-missing25/allRes.rds')